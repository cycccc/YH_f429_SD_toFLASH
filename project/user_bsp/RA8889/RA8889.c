...
//**[02h]**//
    ...
    Data_Format_8b_24bpp();  ///> 主机读写图像数据格式  针对内存  8bit(uint8_t) 图片资源  24位的色深 [2:0]B8 G8 R8
    #endif
    ...
    Data_Format_16b_24bpp_mode1();    ///> 主机读写图像数据格式  针对内存
    #endif
    ...
    MemWrite_Left_Right_Top_Down();//(Default) Memory Write: Left to Right, Top to Down.   内存读法 左--->右;上--->下
...
void System_Check_Temp(void)
{
    ...
    do
    {
        ...
        {
            //delay_ms(1); 					//if MCU too fast//若MCU 速度太快，必要時使用
            LCD_CmdWrite(0x01);
            //delay_ms(1); 					//if MCU too fast//若MCU 速度太快，必要時使用
            temp = LCD_DataRead();
...
//	//#ifdef RAIO_AE
//		//case2:  using DMA Function Control Register
//		//	Check_Busy_SFI_DMA();
//		LCD_CmdWrite(0xB6);
//			do{
//				temp=LCD_DataRead();
...
//		//case3: using BTE Function Control Register
//			do{
//				LCD_CmdWrite(0x90);
//			    temp = LCD_DataRead();
//			}while(temp&0x10);
...
void Set_LCD_Panel(void)
{
    ...
    */
    ...
    LCD_HSYNC_Start_Position(210);//INNOLUX800x600，16~354。  
    LCD_HSYNC_Pulse_Width(10);//INNOLUX800x600，1~40C         
...
//	//--------------//
    ...
    LCD_HorizontalWidth_VerticalHeight(800, 480); //分辨率800x480。

    /*	[16h][17h] : Figure 19-3 [HND]	Non Display or Back porch (pixels)		= (HNDR + 1) * 8 + HNDFTR
    	[18h] :		 Figure 19-3 [HST]	Start Position or Front porch (pixels)	= (HSTR + 1)x8
    	[19h] :		 Figure 19-3 [HPW]	Pulse Width(pixels)						= (HPW + 1)x8
    */
    LCD_Horizontal_Non_Display(46);//INNOLUX800x600，46。       ///> HBP
    LCD_HSYNC_Start_Position(210);//INNOLUX800x600，16~354。    ///> HFP
    LCD_HSYNC_Pulse_Width(20);//INNOLUX800x600，1~40C            ///> HSW

    /*	[1Ch][1Dh] : Figure 19-3 [VND]	Non-Display Period(Line)	= (VNDR + 1)
    	[1Eh] :		 Figure 19-3 [VST]	Start Position(Line)		= (VSTR + 1)
    	[1Fh] :		 Figure 19-3 [VPW]	Pulse Width(Line)			= (VPWR + 1)
    */
    LCD_Vertical_Non_Display(23);//INNOLUX800x600，23。        ///> VBP
    LCD_VSYNC_Start_Position(22);//INNOLUX800x600，1~147。     ///> VFP
    LCD_VSYNC_Pulse_Width(10);//INNOLUX800x600，1~20C           ///> VSW
...
void RA8889_PLL(unsigned short DRAM_clock, unsigned short CORE_clock, unsigned short SCAN_clock)
{
    ...
    */
    ...
    USER_WARNING_REF(x_Divide);
    USER_WARNING_REF(PLLC1);
    USER_WARNING_REF(PLLC2);
    USER_WARNING_REF(pll_m_lo);
    USER_WARNING_REF(pll_m_hi);
    USER_WARNING_REF(temp);
...
/**
  * 函数功能: 读取寄存器地址的值
  * 输入参数: _ucRegAddr：寄存器地址
  * 返 回 值: value：寄存器内容
  * 说    明: 无
  */
static uint8_t RA8889_ReadReg(uint8_t _ucRegAddr)
{
	uint8_t value;
  LCD_DataWrite(_ucRegAddr);  /* 设置寄存器地址 */
	value = LCD_StatusRead();		/* 读取寄存器值 */
	return value;
}
...
/**
  ...
  * 说    明: 无
  */
uint16_t RA8889_ReadID(void)
{
	return RA8889_ReadReg(0x00);
}
